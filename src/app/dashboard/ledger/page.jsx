...
export default async function LedgerPage() {
    ...
    let serializedCustomers = [];
    let totalCount = 0;
    let initialTotals = { debit: 0, credit: 0 };
    ...
    try {
        ...
        totalCount = await prisma.ledgerentry.count({
            where: {
                customer: {
                    name: { not: "Cash Account" }
                }
        ...
        serializedEntries = initialEntries.map(entry => ({
            ...entry,
        ...
        serializedCustomers = initialCustomers.map(customer => ({
            ...customer,
            balance: customer.balance ? parseFloat(customer.balance.toString()) : 0
        }));
    } catch (error) {
        console.error("Database connection error on Ledger page:", error);
    }
